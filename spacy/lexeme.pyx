...
from .attrs cimport IS_BRACKET, IS_QUOTE, IS_LEFT_PUNCT, IS_RIGHT_PUNCT, IS_CURRENCY, IS_OOV, IS_ORG
...
cdef class Lexeme:
    ...
    property is_org:
        """RETURNS (bool): Whether the lexeme is a organisation"""
        def __get__(self):
            return Lexeme.c_check_flag(self.c, IS_ORG)

        def __set__(self, bint x):
            Lexeme.c_set_flag(self.c, IS_ORG, x)
